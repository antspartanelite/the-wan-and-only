...
function quizNext(voiceChannel, guild){
    ...
    guild.songNameGuessed = false;
    guild.passes = 0;
    sendMessage("obi", guild.quizMessageChannel, "The song was "+guild.quizQueue[0][0][0]+" by "+guild.quizQueue[0][1][0]);
    ...
    if(guild.quizQueue.length > 0){
        ...
        sendMessage("obi", guild.quizMessageChannel, toSend);
...
function streamMusic(songUrl, voiceChannel, messageChannel, guild, spareLinks=[]) {
    try{
        ...
        if(!guild.quizMessageChannel){
          sendMessage("obi", messageChannel, "now playing "+songUrl);
        }
        ...
        voiceChannel.join().then(connection => {
          ...
          dispatcher.on('finish', end => {
            if(guild.songQueue.length > 0 && guild.isSongPlaying == true){
                ...
                guild.isSongPlaying = false;
                voiceChannel.leave();
                return sendMessage("obi", messageChannel, "The queue has been completed");
            }
          });
            dispatcher.on('error', error => {if(spareLinks.length == 0){sendMessage("obi", messageChannel, "Invalid URL(s)");}else{
                sendMessage("obi", messageChannel, "Url unavailable trying another");
                spareLinks.shift();
...
async function sendMessage(botType, messageChannel, message, file={}){
  //If the message is too long for discord a note will be posted to console and function will return
  if(message.length > 3998){
    console.log("The message to be sent is too long")
    messageChannel.send("", {
    files: [
        "./pictures/visible confusion.png"
    ]
    });
    return;
  }
  ...
  if(botType == "vades"){
    ...
    return messageChannel.send(message, file);
  }
}
...
client.on('message', message => {
    ...
    if(message.content.startsWith("obi!queue add ")){
        ...
        sendMessage("obi", message.channel, "You have added: "+message.content.slice(14)+" to the queue");
    }
        ...
        if(Number.isInteger(Number(message.content.slice(17))) && message.content.slice(17) <= message.guild.songQueue.length && message.content.slice(17) > 0){
          sendMessage("obi", message.channel, "Queue entry "+message.content.slice(17)+" - "+message.guild.songQueue[Number(message.content.slice(17)-1)].slice(9)+ " - has been removed from the queue");
        ...
        sendMessage("obi", message.channel, tmp);
    }
    else if(message.content == "obi!queue clear"){
        message.guild.songQueue = [];
        return sendMessage("obi", message.channel, "The queue has been cleared");
    }
        ...
        return sendMessage("obi", message.channel, "The queue has been completed");
    }
    ...
    if(message.content === "hi" || message.content.indexOf("hello") != -1){
      sendMessage("obi", message.channel, "", {
      files: [
    ...
    if(message.content.indexOf("underestimate") != -1 && message.content.indexOf("power") != -1){
      sendMessage("obi", message.channel, "Don't try it!");
    }
        ...
        for(var i = 0; i < questions.length; i++){
            ...
            sendMessage("obi", message.channel, questions[i]);
        }
    }
    ...
    if(message.content.indexOf("politics") != -1 || message.content.indexOf("politician") != -1 || message.content.indexOf("brexit") != -1 || message.content.indexOf("conservatives") != -1 || message.content.indexOf("republican") != -1 || message.content.indexOf("democrat") != -1){
      sendMessage("obi", message.channel, "Oh no, I'm not brave enough for politics");
    }
    ...
    if(message.content.indexOf("theresa may") != -1 || message.content.indexOf("hillary clinton") != -1 || message.content.indexOf("padme") != -1 || message.content.indexOf("margaret thatcher") != -1 || message.content.indexOf("angela merkel") != -1){
      sendMessage("obi", message.channel, "Don't forget, she's a politician, and they're **not** to be trusted.");
    }
    ...
    sendMessage("vades", message.channel, words.join(" "));
    }

    if (message.content.indexOf("trick") != -1) {
      sendMessage("vades", message.channel, "try spinning that's a good trick!");
    }

    if(message.content === "anakin, i'm appointing you as my personal representative on the jedi council"){
      sendMessage("vades", message.channel, "Me? A Master? I am overwhelmed, sir, but the Council elects its own members. They will never accept this.");
    }

    if (message.content.indexOf("sand") != -1){
      sendMessage("vades", message.channel, "I don't like sand. It's course and rough and irritating, and it gets everywhere");
    }

    if(message.content.indexOf("youngling") != -1){
      sendMessage("vades", message.channel, "", {
      files: [
    ...
    if(message.content.indexOf("lie ") != -1 || message.content.indexOf(" lie") != -1 || message.content == "lie"){
      sendMessage("vades", message.channel, "I see through the lies of the jedi");
    }

    if(message.content.indexOf("evil") != -1){
      sendMessage("vades", message.channel, "from my point of view the jedi are evil");
    }

    if(message.content.indexOf("over anakin") != -1 || message.content.indexOf("i have the high ground") != -1){
      sendMessage("vades", message.channel, "You underestimate my power");
    }

    if(message.content.indexOf("master") != -1){
      sendMessage("vades", message.channel, "It's outrageous it's unfair, how can you have a seat on the council but not be granted the rank of master");
    }
    ...
    if(message.content.indexOf("obiwan") != -1 || message.content.indexOf("obi wan") != -1 || message.content.indexOf("obi-wan") != -1 || message.content.indexOf("kenobi") != -1 || message.content.indexOf("the wan and only") != -1){
      sendMessage("vades", message.channel, "I don't want to hear anymore about obiwan");
    }
    ...
    if(message.content.startsWith("obi!invite")){
      sendMessage("obi", message.channel, "https://discord.com/oauth2/authorize?client_id=806342829995065435&scope=bot&permissions=607636545");
    }
        ...
        sendMessage("obi", message.channel, "Welcome to the wan and only discord music quiz, the aim of the quiz is to guess the author and the song name correctly\nThe quiz will be starting in\n5");
        let i = 4
        var counter = setInterval(() => {
            sendMessage("obi", message.channel, i.toString());
            i-=1;
            if(i==0){
                message.guild.quizQueue = [];
                for(var j = 0; j < 10; j++){
        ...
        for(i in message.guild.playerArray){
            if(message.guild.playerArray[i].Id == message.author.id && message.guild.playerArray[i].Passed == false){
                ...
                sendMessage("obi", message.channel, message.guild.passes.toString()+"/"+Math.ceil((message.guild.playerArray.length/2)+0.0001)+"votes required to pass the song have been cast");
